...
masters_number = config('MASTERS_NUMBER')

# Global Vars
pinger = -1
now = datetime.datetime.now()
start_time = datetime.datetime.now()
COMMANDS = ['status', 'quit', 'ping', 'uptime']
...
@contextmanager
def as_file(url):
    # Download the media as temporary file to send it to contact
...
def response(message):
    answers = requests.post(
        '{}/answer'.format(config('API_URL')),
        json={
            'user_input': message.content,
            'contact': {
                'phone_number': message.sender.id.split('@')[0],
        ...
        for answer in answers.json():
            if answer.get('type') == 'text':
                driver.send_message_to_id(
                    message.sender.id, answer.get('content')[0])
            elif answer.get('type') == 'media':
                with as_file(answer.get('content')) as filename:
                    caption = '' # The caption sent under the image, optional
                    driver.send_media(filename, message.sender.id, caption) # Expected send_media(self, path, chatid, caption)
    else:
        driver.send_message_to_id(
            message.sender.id,
            'Ops, parece que estou tendo alguns problemas de comunicação. \
...
def process_command(command):
    ...
    if command.lower() == 'status':
        ...
        send_message_to_master("Up since {start}, hence for a total time of {upt} by now".format(
...
if config('MODE') == 'production':
    ...
    display.start()
driver = WhatsAPIDriver(username='Sorrisus', loadstyles=True,
        ...
        for contact in driver.get_unread(include_me=False, include_notifications=True):
            for message in contact.messages:
                ...
                if message.type == 'chat':
                    ...
                    f.write("[ {sender} | {timestamp} ] ".format(
